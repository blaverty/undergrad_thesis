    ...
    for head, lines in groupby(infile, ishead):
        if head:
            ...
            if names != set():
                ...
                for hit in hits:
                    ...
                    if namenot not in names:
                        if namenot not in div:
                            if namenot not in open('/home/brianne/sepsis2/Card/geneFamily/nucOrder/all/'+fasta).read():
                ...
                hits = hits.split("\n")
                for hit in hits:
                    name_no_snp = hit.split("|")[4]
                    if name_no_snp in open('/home/brianne/sepsis2/Card/geneFamily/nucOrder/all/'+fasta).read():
                        print(name_no_snp, "hitt")
                    else:
                        print(name_no_snp, "ranhit")
